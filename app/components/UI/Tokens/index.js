...
class Tokens extends PureComponent {
	...
	renderItem = (asset) => {
		...
		if (chainId === '51112') {
			try{
				if (asset.symbol === 'CC') {
					var sbCc = parseFloat(mainBalance.split(' ')[0]) * parseFloat(tokenExchangeRates["0xcca0818689680b7686cc192035ed23c8843c6892"].price);
					secondaryBalance = (sbCc > 0 ? sbCc.toFixed(4) : 0) + " $";
				} else if (itemAddress && tokenExchangeRates.hasOwnProperty(itemAddress.toLowerCase())) {
					var sbOther = parseFloat(mainBalance.split(' ')[0]) * parseFloat(tokenExchangeRates[itemAddress.toLowerCase()].price);
					secondaryBalance = (sbOther > 0 ? sbOther.toFixed(4) : 0) + " $";
				} else {
					secondaryBalance = "0 $";
				}
			}catch(e){
		...
		return (
			...
			>
				{asset.isETH ? (
					<NetworkMainAssetLogo medium style={styles.ethLogo} testID={'eth-logo'} />
				) : (
					<TokenImage asset={asset} containerStyle={styles.ethLogo} iconStyle={styles.ethLogo} />
				)}
				<View style={styles.symbolText}>
					<Text style={styles.symbolText2}>{mainBalance.split(' ')[1]}</Text>
				</View>
	...
	render = () => {
		...
		return (
			<View style={styles.wrapper} testID={'tokens'}>
				<View><Text></Text></View>
				{(false && chainId === '51112') ? (<Text style={styles.text2}><Text style={styles.text3}>{strings('transaction.asset')} : </Text>{this.worth.toFixed(4)} <Text style={styles.text3}>$</Text></Text>):(1==1)}
