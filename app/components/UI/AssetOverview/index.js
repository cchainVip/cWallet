...
class AssetOverview extends PureComponent {
	...
	renderLogo = () => {
		...
		if (!iconUrl && address) {
			iconUrlTmp = 'https://d2h8jcrpwh6m7t.cloudfront.net/tokens/' + address.toLowerCase() + '.png';
		}
	...
	renderWarning = () => {
		...
		const supportArticleUrl = AppConstants.zhome + 
			'/pages/balance';
		return (
	...
	render() {
		...
		if ((chainId === '1' && symbol === 'ETH') || (chainId === '56' && symbol === 'BNB') || (chainId === '128' && symbol === 'HT') || (chainId === '66' && symbol === 'OKT') || (chainId === '51112' && symbol === 'CC')) {
			showTokenAddress = false;
		} // zyl
		let showTokenSwap = false;
		if (chainId === '51112' && symbol === 'USDT') {
			showTokenSwap = true;
		}

		return (
			<View style={styles.wrapper} testID={'token-asset-overview'}>
				<TouchableOpacity onPress={() => this.goToBrowserUrlSimple(AppConstants.zhome + "/pages/token?chain=" + chainId + "&address=" + address, symbol)}>
				<View style={styles.assetLogo}>{this.renderLogo()}</View>
				</TouchableOpacity>
				{showTokenAddress && (<TouchableOpacity style={styles.addressWrapper} onPress={this.copyAccountToClipboard}>
					<Text style={styles.copyColor}>{strings('asset_overview.copyTokenAddress')}</Text><EthereumAddress onPress={this.copyAccountToClipboard} address={address} style={styles.address} type={'short'} />
				</TouchableOpacity>)}
