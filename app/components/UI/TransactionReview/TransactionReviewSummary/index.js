...
class TransactionReviewSummary extends PureComponent {
	static propTypes = {
		...
		fiatValue2: PropTypes.string,
	};

	renderWarning = () => <Text>{`${strings('transaction.approve_warning')} ${this.props.assetAmount}`}</Text>;

	render = () => {
		const { actionKey, assetAmount, conversionRate, fiatValue, approveTransaction, primaryCurrency, fiatValue2 } = this.props;
		return (
			<View>
				{!!approveTransaction && (
				...
				<View style={styles.summary}>
					...
					{!conversionRate ? (
						<View>
							...
							<Text style={styles.summarySecondary}>{fiatValue2}</Text>
						</View>
					) : (
						<View>
							<Text style={styles.summaryPrimary}>
								{primaryCurrency === 'ETH' ? assetAmount : fiatValue2}
							</Text>
							<Text style={styles.summarySecondary}>
								{primaryCurrency === 'ETH' ? fiatValue2 : assetAmount}
							</Text>
						</View>
					)}
				</View>
