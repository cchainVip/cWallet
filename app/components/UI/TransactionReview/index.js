...
class TransactionReview extends PureComponent {
	...
	render = () => {
		...
		let fiatValue2 = 0;
		try {
			const { contractExchangeRates} = this.props;
			if (transaction.assetType == "ETH") {
				fiatValue2 = parseFloat(contractExchangeRates["0xcca0818689680b7686cc192035ed23c8843c6892"].price) * parseFloat(transaction.readableValue);
				fiatValue2 = fiatValue2.toFixed(6) + " $";
			} else if (transaction.assetType == "ERC20") {
				fiatValue2 = parseFloat(contractExchangeRates[transaction.selectedAsset.address.toLowerCase()].price) * parseFloat(transaction.readableValue);
				fiatValue2 = fiatValue2.toFixed(6) + " $";
			} else {
				fiatValue2 = fiatValue;
			}
		} catch(err) {
			console.log(err)
		}
