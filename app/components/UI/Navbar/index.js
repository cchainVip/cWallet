...
export function getBrowserViewNavbarOptions(navigation, route) {
	...
	const isHomepage = (url) => getHost(url) === getHost(AppConstants.zhome);
	...
	if (url && !isHomepage(url)) {
		...
		// zyl 隐藏发现页dapp的网址，显示为title step2
		host = route.params?.title ?? '';
	} else {
	...
	function onPress() {
		...
		// navigation.openDrawer();
		// trackEvent(ANALYTICS_EVENT_OPTS.COMMON_TAPS_HAMBURGER_MENU);
		DeviceEventEmitter.emit('zshowSwitch', "toggle");
	}
	...
	return {
		...
		headerLeft: () => (
			<View style={styles.browserRightButton}>
				<TouchableOpacity onPress={onPress2} style={styles.hamburgerButton} testID={'hamburger-menu-button-browser'}>
					<IonicIcon
						name={Device.isAndroid() ? 'md-wallet' : 'ios-menu'}
						size={Device.isAndroid() ? 24 : 28}
						style={styles.backIcon}
					/>
				</TouchableOpacity>
...
export function getOnboardingNavbarOptions(navigation, route, { headerLeft } = {}) {
	...
	return {
		...
		headerRight: () => <View />,
		headerLeft: () => (<TouchableOpacity onPress={() => navigation.pop()} style={styles.backButton}>
