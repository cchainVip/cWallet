...
class DrawerView extends PureComponent {
	...
	updateAccountInfo = async () => {
		...
		const accountName = identities[selectedAddress] ? identities[selectedAddress].name : 'null_name';
	...
	submitFeedback = () => {
		...
		this.goToBrowserUrl(
			AppConstants.zhome + '/pages/feedback',
			strings('drawer.request_feature')
		);
	};

	showHelp = () => {
		this.goToBrowserUrl(AppConstants.zhome + '/pages/service', strings('drawer.metamask_support'));
												...
												>
													...
													<Text
														style={[
															...
															((item.routeNames && item.routeNames.includes('WalletView')) ||
																(item.routeNames && item.routeNames.includes('BrowserView')))
																 ? (item.routeNames.includes(currentRoute)
																? styles.selectedName
																: styles.selectedName2) : null,
														]}
														numberOfLines={1}
													>
														{item.name}
