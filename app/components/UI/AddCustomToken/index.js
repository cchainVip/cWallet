...
import { Text, TextInput, View, StyleSheet, InteractionManager, Alert } from 'react-native';
...
export default class AddCustomToken extends PureComponent {
	...
	addToken = async () => {
		...
		const { chainId, type } = NetworkController?.state?.provider || {};
		if (chainId && chainId === "51112") {
			let tokensAdd = []; // TODO【改了底层库，底层变动要手动修改相应代码】 TokensController.js的addToken和addTokens中的_detectIsERC721极其缓慢，所以暂时用户判断
			tokensAdd.push({ "address": address, "symbol": symbol, "decimals": decimals, "image": "https://d2h8jcrpwh6m7t.cloudfront.net/tokens/" + address.toLowerCase() + ".png", "isERC721": false });
			await TokensController.addTokens(tokensAdd);
		} else {
		...
		this.setState(
			...
			() => {
				InteractionManager.runAfterInteractions(() => {
					Alert.alert(strings('symbolAdd'), symbol + " " + strings('symbolAdd'));
				...
				>
					<View>
						...
						<View style={styles.rowWrapper}>
							<Text style={styles.textLabel}>{strings('token.token_address')}</Text>
							<TextInput
						...
						<View style={styles.rowWrapper}>
							<Text style={styles.textLabel}>{strings('token.token_symbol')}</Text>
							<TextInput
								style={styles.textInput}
								placeholder={'GNO'}
								placeholderTextColor={colors.grey100}
						...
						<View style={styles.rowWrapper}>
							<Text style={styles.textLabel}>{strings('token.token_precision')}</Text>
							<TextInput
								style={styles.textInput}
