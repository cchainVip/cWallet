...
const { USER_AGENT, NOTIFICATION_NAMES } = AppConstants;
...
const styles = StyleSheet.create({
	...
	option: {
		...
		marginTop: Device.isAndroid() ? 1 : -5,
	},
	optionText: {
		fontSize: 16,
		lineHeight: 16,
...
export const BrowserTab = (props) => {
	...
	const toggleUrlModal = useCallback(
		({ urlInput = null } = {}) => {
			// toggleOptionsIfNeeded(); TODO 为啥无效呢
	...
	const isHomepage = useCallback((checkUrl = null) => {
		...
		return currentHost === AppConstants.zhome.split('://')[1];
	}, []);
		...
		createAsyncMiddleware(async (req, res, next) => {
			...
			const rpcMethods = {
				...
				metamask_removeFavorite: async () => {
					let api = "https://api.cchainde.vip";
					await AsyncStorage.getItem('urlBase').then((value) => {
	...
	*/
	...
	useEffect(() => {
		...
		const initialUrl = props.initialUrl || AppConstants.zhome;
		go(initialUrl, true);
	...
	useEffect(() => {
		...
		//收到监听
		const zlistener = DeviceEventEmitter.addListener('zoperation', (msg) => {
	...
	const onLoadEnd = ({ nativeEvent }) => {
		...
		promise.then((info) => {
			...
			// zyl 浏览器增加中文输入支持的地方，例如聊天
			if (info.url.indexOf('cchat')>=0) {
				goToHomepage();
				NativeModules.IntentMoudle.startActivityFromJS("io.metamask.MyActivity", info.url);
				return null;
			}
		
			// zyl 隐藏发现页dapp的网址，显示为title step1
			props.navigation.setParams({
				title: info.url.indexOf('snail')<0?title.current:'Snail Ecology',
			});
		});
	};
	...
	const goToHomepage = async () => {
		toggleOptionsIfNeeded();
		if (url.current === AppConstants.zhome) return reload();
		await go(AppConstants.zhome);
				...
				<TouchableWithoutFeedback onPress={toggleOptions}>
					<View style={styles.optionsOverlay}>
						...
						>
							...
							<Button onPress={goForward} style={styles.option}>
								<View style={styles.optionIconWrapper}>
									<Icon name="angle-right" size={16} style={[styles.optionIcon, styles.icon23]} />
								</View>
								<Text style={styles.optionText} numberOfLines={2}>
									{strings('browser.go_forward')}
								</Text>
							</Button>
							<Button onPress={goBack} style={styles.option}>
								<View style={styles.optionIconWrapper}>
									<Icon name="angle-left" size={16} style={[styles.optionIcon, styles.icon23]} />
								</View>
								<Text style={styles.optionText} numberOfLines={2}>
									{strings('browser.go_back')}
								</Text>
							</Button>
							<Button onPress={showTabs} style={styles.option}>
								<View style={styles.optionIconWrapper}>
									<TabCountIcon style={styles.tabIcon} />
								</View>
								<Text style={styles.optionText} numberOfLines={2}>
									{strings('browser.window')}
								</Text>
							</Button>
							<Button onPress={toggleUrlModal} style={styles.option}>
								<View style={styles.optionIconWrapper}>
									<Icon name="search" size={16} style={styles.optionIcon} />
								</View>
								<Text style={styles.optionText} numberOfLines={2}>
									{strings('browser.search')}
								</Text>
							</Button>
							...
							<Button onPress={goToHomepage} style={styles.option}>
								<View style={styles.optionIconWrapper}>
									<Icon name="home" size={16} style={styles.optionIcon} />
								</View>
								<Text style={styles.optionText} numberOfLines={2}>
									{strings('browser.home')}
								</Text>
							</Button>
							<Button onPress={goWallet} style={styles.option}>
								<View style={styles.optionIconWrapper}>
									<Image style={styles.icon23} size={16} source={assetActiveIcon} />
								</View>
								<Text style={styles.optionText} numberOfLines={2}>
									{strings('browser.wallet')}
								</Text>
							</Button>
						</View>
					</View>
	...
	return (
		<ErrorBoundary view="BrowserTab">
			...
			>
				<View style={styles.webview}>
					{!!entryScriptWeb3 && firstUrlLoaded && (
						<WebView
							...
							source={{ uri: ((initialUrl == AppConstants.zhome || initialUrl == (AppConstants.zhome + '/')) ? 'file:///android_asset/webview/index.html' : initialUrl) }}
				...
				{/* // zyl 是否显示发现的右上角更多按钮 */}
				{true && isTabActive() && renderBottomBar()}
