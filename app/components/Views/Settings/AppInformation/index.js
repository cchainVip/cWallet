...
export default class AppInformation extends PureComponent {
	...
	onAttributions = () => {
		const url = AppConstants.zhome + '/pages/attributions';
		this.goTo(url, strings('app_information.attributions'));
	};

	onSupportCenter = () => {
		const url = AppConstants.zhome + '/pages/support';
		this.goTo(url, strings('drawer.metamask_support'));
	};

	onWebSite = () => {
		const url = AppConstants.zhome;
		this.goTo(url, AppConstants.zhome.split('://')[1]);
	};

	onContactUs = () => {
		const url = AppConstants.zhome + '/pages/contact';
		this.goTo(url, strings('app_information.contact_us'));
	};
