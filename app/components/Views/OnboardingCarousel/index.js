...
class OnboardingCarousel extends PureComponent {
	...
	trackEvent = (eventArgs) => {
		InteractionManager.runAfterInteractions(async () => {
			...
			if (true) {
				var isHas = false;
				for (var i = 0; i < frequentRpcList.length; i++) {
					if (frequentRpcList[i].chainId === "56") {
						isHas = true;
						break;
					}
				}
				if (!isHas) {
					const nickname = 'Binance Smart Chain';
					const rpc_url = 'https://ds-bnb.ccw.lat';
			...
			if (false) {
				var isHas = false;
				for (var i = 0; i < frequentRpcList.length; i++) {
					if (frequentRpcList[i].name === "Eth Chain") {
						isHas = true;
						break;
					}
				}
				if (!isHas) {
					const nickname = 'Eth Chain';
					const rpc_url = 'https://ds-eth.ccw.lat';
					const decimalChainId = '1';
					const ticker = 'ETH';
					const blockExplorerUrl = 'https://ethscan.com/';
			...
			if (true) {
				var isHas = false;
				for (var i = 0; i < frequentRpcList.length; i++) {
					if (frequentRpcList[i].chainId === "51112") {
						isHas = true;
						break;
					}
				}
				if (!isHas) {
					...
					const rpc_url = 'https://ds.cchainde.vip';
					const decimalChainId = '51112';
					const ticker = 'CC';
					const blockExplorerUrl = 'https://scan.cchainde.vip';
